# Google Skills Scraper — desktop GUI (Tauri)

A thin [Tauri](https://tauri.app) desktop shell over the `skills-scraper` Go binary. Every
action shells out to `skills-scraper` (see `src-tauri/src/main.rs`), so the GUI inherits the
...
- The **`skills-scraper` binary** built and reachable (see below).
- **Google Chrome** (the fetch/login commands drive it, via `skills-scraper`).

The frontend has no build step (plain HTML/JS), so Node is optional — the Tauri
CLI can run via cargo.

## Build the `skills-scraper` binary first

The GUI resolves the binary in this order: `CSB_BIN` env → `<repo-root>/skills-scraper.bin`
(or `skills-scraper.exe`, plus legacy `csb.bin`/`csb`). The simplest setup drops it at the repo root — the
`.bin` extension keeps it out of git via a single `*.bin` ignore:

```bash
cd go && go build -o ../skills-scraper.bin . && cd ..    # or: just cli
```

It also runs `skills-scraper` with the working directory set to the repo root (detected by
...
just dev              # builds the skills-scraper binary, then launches the desktop app
```

`just dev` builds `skills-scraper.bin` and then runs `cargo tauri dev` for you.
...
cd go && go build -o ../skills-scraper.bin . && cd .. # build the binary the GUI calls
...
- **Browse** — list stored paths / courses / labs (`skills-scraper list --json`), each with
  ...
  `skills-scraper list --reload`) — needed once on first run to populate an empty database.
- **Search** — query the local database (`skills-scraper search --json`), with the same
  fetch-status badges.
- **Sign in** — opens a browser via `skills-scraper login`; click *Done* when finished.
- **Open vault** — reveals `csbmdvault/` in the OS file manager.

## How it talks to `skills-scraper`

The read commands use `skills-scraper … --json` (clean JSON on stdout, status on stderr).
